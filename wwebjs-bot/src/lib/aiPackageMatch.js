...
const QUANTITY_SYSTEM_PROMPT = `You extract how many units of a known product are ordered from a WhatsApp item line (Cameroon context).
Return JSON only: { "quantity": integer, "confidence": number 0-1 }
Rules:
- quantity = units of the named product to ship (not phone, not price in FCFA).
- "3 Acide Glycolique" → 3, "Acide x2" → 2, "2x robes" → 2, "qté 2" → 2.
- If no quantity is stated, return quantity 1 with confidence 0.9.
- Never return quantity > 99 unless clearly stated.
- Respond with JSON only, no markdown.`;

/**
 * AI fallback when regex quantity parsing is ambiguous (stock orders are quantity-sensitive).
 * @param {string} itemLine - full parsed items line from WhatsApp
 * @param {string} knownPackageName - matched catalog package_name
 * @param {object} config
 * @returns {Promise<{ quantity: number, confidence: number }|null>}
 */
async function extractQuantityWithAI(itemLine, knownPackageName, config) {
  if (!itemLine || !knownPackageName || !config?.OPENAI_API_KEY) {
    return null;
  }
  ...
  try {
    ...
    return { quantity: qty, confidence };
  } finally {
    clearTimeout(timeout);
  }
}

module.exports = { matchPackageWithAI, extractQuantityWithAI };
